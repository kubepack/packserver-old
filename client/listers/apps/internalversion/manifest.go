...
type ManifestLister interface {
	...
	List(selector labels.Selector) (ret []*apps.Manifest, err error)
...
// List lists all Manifests in the indexer.
func (s *manifestLister) List(selector labels.Selector) (ret []*apps.Manifest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*apps.Manifest))
	})
	return ret, err
}
...
type ManifestNamespaceLister interface {
	...
	Get(name string) (*apps.Manifest, error)
...
// List lists all Manifests in the indexer for a given namespace.
func (s manifestNamespaceLister) List(selector labels.Selector) (ret []*apps.Manifest, err error) {
...
// Get retrieves the Manifest from the indexer for a given namespace and name.
func (s manifestNamespaceLister) Get(name string) (*apps.Manifest, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(apps.Resource("manifest"), name)
	}
	return obj.(*apps.Manifest), nil
}
