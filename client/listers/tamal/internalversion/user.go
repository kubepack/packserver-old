...
type UserLister interface {
	...
	List(selector labels.Selector) (ret []*tamal.User, err error)
	// Get retrieves the User from the index for a given name.
	Get(name string) (*tamal.User, error)
	UserListerExpansion
}
...
// List lists all Users in the indexer.
func (s *userLister) List(selector labels.Selector) (ret []*tamal.User, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*tamal.User))
	})
	return ret, err
}

// Get retrieves the User from the index for a given name.
func (s *userLister) Get(name string) (*tamal.User, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(tamal.Resource("user"), name)
	}
	return obj.(*tamal.User), nil
}
